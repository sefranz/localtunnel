...
const debug = require('debug')('lt:client');
...
module.exports = class Tunnel extends EventEmitter {
  ...
  _init(cb) {
    ...
    // where to quest
    const uri = `${baseUri}?new=${assignedDomain}`;
  ...
  _establish(info) {
    ...
    const { max_conn } = info;
    const { defaultMaxListeners = 10 } = EventEmitter;
    this.setMaxListeners(parseInt(max_conn, 10) + defaultMaxListeners)
